...
func TestMainCapabilities(t *testing.T) {
	...
	if !got.OK || got.Version != version || !got.BatchInsertAfter || !got.BatchUpdatedAnchors {
		t.Fatalf("capabilities = %#v, want current version with batch insert-after and updated anchors", got)
	}
}
