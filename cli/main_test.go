...
func TestMainCapabilities(t *testing.T) {
	...
	if !got.OK || got.Version != version || !got.BatchInsertAfter || !got.BatchUpdatedAnchors || !got.ReadRangeMetadata {
		t.Fatalf("capabilities = %#v, want current version with batch insert-after, updated anchors, and read-range metadata", got)
	}
}
