...
stale 的 remap 只用于定位，不能自动替换后重试。stale batch 必须携带 `currentAnchors`，其内容来自拒绝该 batch 时的同一文件快照；插件会把快照放入正文与 `details.error.currentAnchors`，并根据失败 change 在正文和 `details.error.staleAnchors` 中列出字段、提交锚点、当前同号行锚点及当前文本。重复 remap 在正文中只渲染一次。调用方只能在未截断窗口仍明确覆盖原定目标及完整范围时显式提交新锚点；上下文缺失、截断或无法确认范围时必须调用 `hledit_read_anchors`。任何路径都不得自动修正、重试或覆盖并发修改。
...
测试必须覆盖 capability、四种严格 operation、旧参数拒绝、最多两层的结构化参数解包、带源码锚点标注归一化、结构化读取的实际范围/总行数/EOF/续读/单行截断/越界错误、batch 与 check 协议、未知 JSON 字段拒绝、UTF-8/BOM 边界、单行范围重复护栏、紧接下一行的显式 delete 放行、stale 优先级、stale 同快照字段核对及零写入、重复 remap 去重、无占位 anchor 的恢复正文、真实 `insert_after` lines、相邻 `delete_range` 合并模板、新锚点结构、中文折叠错误摘要、多操作范围标题、prompt guideline 工具名、tool error 升级，以及 bundled CLI 与插件的端到端调用。
