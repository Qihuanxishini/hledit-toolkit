...
- `replace` 未提供 `end_anchor` 时只消费一个源文件行，`lines` 有多行也不会隐式覆盖后续源行；
- 单锚点多行 `replace` 若首个输出行与原锚点行完全相同，插件在调用 CLI 前拒绝；保留锚点行并新增内容应使用 `insert`，替换原有块应使用 `end_anchor`；
...
- 插件在 CLI 前只执行语义明确的单锚点重复护栏；不会猜测、补全或静默丢弃 change；
...
TUI 渲染由插件自身完成，不通过全局 adapter API 委托给其他扩展。读取结果以 `details.read` 为真源，工具标题仍显示请求范围，结果摘要显示实际范围、总行数、EOF 或下一 offset；路径在终端支持时使用 `file://` 超链接。diff 组件必须在每次 `render(width)` 时按 120 列断点重新选择双栏或统一布局；新增/删除背景色由当前 `toolSuccessBg` 与 `toolDiffAdded` / `toolDiffRemoved` 混合生成，以适配深浅主题。
...
| `src/result.ts` | 验证结构化读取/编辑响应，生成模型正文、disposition、actionable error、stale 指引和 diff details。 |
...
capabilities 必须同时返回 `readRangeMetadata:true`、`batchInsertAfter:true` 和 `batchUpdatedAnchors:true`。
