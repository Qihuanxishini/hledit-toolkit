...
// Pi 内置 compaction 文件提取只识别名为 read/write/edit 的工具调用参数；三个
...
// - 读取成功 → read；
// - apply/replace-once 成功且内容变更 → edited；
...
export function recordAnchoredFileOperations(messages: unknown[], fileOps: AnchoredCompactionFileOps): void {
	for (const message of messages) {
		...
		if (candidate.toolName !== HLEDIT_APPLY_FILE_CHANGES_TOOL && candidate.toolName !== HLEDIT_REPLACE_ONCE_TOOL) continue;
