...
export type DiffRenderComponent = {
...
function resolveDiffBackgroundPalette(theme: DiffRenderTheme): DiffBackgroundPalette | undefined {
...
function parseGeneratedDiff(diff: string): ParsedDiff {
	...
	for (const rawLine of diff.split("\n")) {
		const displayLine = rawLine.replace(/\r/g, "␍");
		if (!displayLine && entries.length === 0) continue;
		const match = GENERATED_DIFF_LINE.exec(displayLine);
		if (!match) {
			entries.push({ kind: "meta", content: displayLine.trim() || "…" });
...
function applyLineLimit(lines: string[], expanded: boolean, width: number, theme: DiffRenderTheme): string[] {
